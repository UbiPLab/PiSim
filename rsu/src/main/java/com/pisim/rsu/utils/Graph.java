...
public class Graph {
    ...
    //删除节点
    public void removeVertex(Vertex v, Iterator<Vertex> iterator1) {
        //从节点集中删除该节点
        //vertexList.remove(v);
        iterator1.remove();
        //从节点对应的边集中删除对应的边
        ...
        while (iterator.hasNext()) {
            ...
            while (it.hasNext()) {
                Edge edge = it.next();
                if (edge.getOneVertex().equals(v) || edge.getAnotherVertex().equals(v)) {
                    it.remove();
                }
            }
    ...
    //将初始的信任种子均分给指定的信任节点
    public void setVertexTrustValue(double trustSeed, List<Graph.Vertex> verList) {
    ...
    //按照司机的pid给指定的单个司机分配信任值
    public void setVertexTrustValueByPid(String pid, double trustValue) {
        List<Graph.Vertex> vertexList = this.vertexList;    //图中所有节点的列表
        boolean hasVertex = false;  //对于给定的pid，是否有对应的司机的标志

        for (Vertex vertex : vertexList) {
            if (pid.equals(vertex.getId())) {
                vertex.setTrustValue(trustValue);
                System.out.println("分配给节点"+vertex.getId()+"信任值:"+trustValue);
                hasVertex = true;
                break;
            }
        }
        if (hasVertex == false) {
            System.out.println("找不到对应的司机，分配信任值失败！(pid=" + pid + ")");
        }
    }

    //按照给定的司机的pid集合给对应的司机列表分配信任值，参数中的信任值平均分配给pid集合中的所有司机
    public void setTrustValueByPidList(List<String> pidList, double trustSeed) {
        List<Graph.Vertex> vertexList = this.vertexList;    //图中所有节点的列表
        double trustValue = trustSeed / pidList.size();     //每个司机的信任值
        boolean hasVertex = false;  //对于给定的pid，是否有对应的司机的标志
    ...
    public void filterMaliciousVertex() {
        ...
        while (iterator.hasNext()) {
            ...
            if (vertex.getTrustValue() < cutoffThreshold) {
                removeVertex(vertex, iterator);
            }
        }
    }
